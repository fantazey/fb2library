...
class Genre(models.Model):
    ...
    name = models.CharField(u'Название жанра', max_length=300, null=True, blank=True)
...
class Author(models.Model):
    ...
    middle_name = models.CharField(u'Отчество', max_length=200, null=True, blank=True)
...
class SequenceBook(models.Model):
    ...
    def __unicode__(self):
        return "%s - %s (%s)" % (self.book.title, self.sequence.name, self.number)
...
class Book(models.Model):
    ...
    authors = models.ManyToManyField(Author, verbose_name=u'Авторы', related_name='authors')
    lang = models.ForeignKey(Language, verbose_name=u'Язык', null=True, related_name=u'book_lang')
    src_lang = models.ForeignKey(Language, verbose_name=u'Язык оригинала', null=True, related_name=u'book_src_lang')
    date = models.CharField(u'Дата', max_length=50, null=True, blank=True)
    sequence = models.ManyToManyField('Sequence', verbose_name=u'Серии книги', null=True, through='SequenceBook')
    book_file = models.CharField(u'Файл', max_length=1000)
    image = models.ImageField(u'Обложка', upload_to='covers/', null=True, blank=True, max_length=300)
    ...
    def add_sequences(self, sequence_list):
        for _seq, num in sequence_list:
            try:
                ...
                num = None
            if not num and not SequenceBook.objects.filter(book=self, sequence=_seq, number=num).exists():
